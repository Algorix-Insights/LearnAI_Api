...
CREATE TABLE study_rooms_messages (
    ...
    updated_at timestamptz NOT NULL DEFAULT now(),
    deleted_at timestamptz,
    CONSTRAINT study_rooms_messages_type_chk CHECK (message_type IN ('text', 'file', 'system', 'ai')),
    CONSTRAINT study_rooms_messages_role_chk CHECK (role IN ('system', 'user', 'assistant', 'tool')),
    CONSTRAINT study_rooms_messages_tokens_chk CHECK (
...
CREATE INDEX idx_study_rooms_messages_study_room_id ON study_rooms_messages(study_room_id);
CREATE INDEX idx_study_rooms_messages_user_id ON study_rooms_messages(user_id);
CREATE INDEX idx_study_rooms_messages_created_at ON study_rooms_messages(created_at);
...
CREATE TRIGGER set_study_rooms_messages_updated_at
BEFORE UPDATE ON study_rooms_messages
FOR EACH ROW EXECUTE FUNCTION set_updated_at();
